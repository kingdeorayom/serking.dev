...
import image1 from '@/images/photos/serking_1.jpg'
import image2 from '@/images/photos/serking_2.jpg'
import image3 from '@/images/photos/serking_3.jpg'
import image4 from '@/images/photos/serking_4.jpg'
import image5 from '@/images/photos/serking_5.jpg'
      ...
      <ol className="mt-6 space-y-4">
        {education.map((item, index) => (
          <li key={index} className="flex gap-4">
            <div className="relative mt-1 flex h-10 w-10 flex-none items-center justify-center rounded-full shadow-md shadow-zinc-800/5 ring-1 ring-zinc-900/5 dark:border dark:border-zinc-700/50 dark:bg-zinc-800 dark:ring-0">
              <Image src={item.logo} alt="" className="h-9 w-9 rounded-full" unoptimized />
            </div>
            <dl className="flex flex-auto flex-wrap gap-x-2">
              ...
              <dd
                className="ml-auto text-xs text-zinc-500 dark:text-zinc-500"
                aria-label={`${item.start.label ?? item.start} until ${item.end.label ?? item.end
                  }`}
              >
                <time dateTime={item.start.dateTime ?? item.start}>
                  {item.start.label ?? item.start}
                </time>{' '}
                <span aria-hidden="true">—</span>{' '}
                <time dateTime={item.end.dateTime ?? item.end}>
                  {item.end.label ?? item.end}
                </time>
              </dd>
            </dl>
          </li>
